...
# Get Hugging Face token and model ID
...
def load_pipeline_with_lora(model_id: str, lora_repo: str = None):
    """Load base pipelines, and apply LoRA if provided."""
    ...
    if lora_repo:
        ...
        unet_base = UNet2DConditionModel.from_pretrained(
            model_id, subfolder="unet", use_auth_token=HF_TOKEN, torch_dtype=torch.float16
        )
...
def save_image_to_tmp_and_get_path(image: Image.Image) -> str:
    """Save image to /tmp and return its path."""
